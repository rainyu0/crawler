...
def down_know_analysis(get_params):
    url_parser = urlParser(mfg_config.knowledge_analysis_path)
    for pr in get_params:
        url_parser.setParams(pr, get_params[pr])
    result_json = common_down(url_parser)
    if result_json['status'] != '11-001':
        Log.error('modify book failed:' + url_parser.getFullUrl())
        return None
    return result_json['result']['pointInfo']


def crawl_know_desc(q_analysis, subject, grade_id):
    know_list = re.findall(r"onclick=GetKnowledgeAnalysis\(([^\)]*)\)", q_analysis)
    for id_str in know_list:
        items = id_str.split(',')
        k_id = items[0]
        k_name = items[1][1:-1]
        ...
        if e_know_id in Downed_Know:
            continue
        Downed_Know[e_know_id] = 1
        old_subject = mfg_config.compare_check['in_subject'][subject]
        ...
        pointInfo = down_know_analysis(know_params)
        if pointInfo:
            new_pointInfo = dispose_img(pointInfo, grade_id)
            insert_know_desc(e_know_id, k_name, k_id, new_pointInfo)

def insert_know_desc(id, name, old_id, detail):
    sql = 'insert into know_desc(id, name, old_id, detail) values(%s, %s, %s, %s) ' \
          'on duplicate key update name = %s, old_id = %s, detail = %s'
    params_con = (id, name, old_id, detail, name, old_id, detail)
...
def insert_one(one_question):
    ...
    crawl_know_desc(new_analysis, one_question['subject'], one_question['grade_id'])
    ...
    sql = 'insert into question_bank(question_id, question_tag, content, question_answer, subject, type,' \
          ...
          ' on duplicate key update grade_id = %s, term_id = %s, old_id = %s'
...
def dispose_books(books, subject, old_grade):
    ...
    for book in book_list:
        if (book['isSelect'] == False) and (book['bookId'] != ''):
...
def get_book_ids(subject, grade_id):
    books = {}
    conn = Mysql_Pool.get_conn()
    sql = "select * from book_info where subject = " + str(subject) + " and  book_version != '' "
    if grade_id < 10:
...
def get_versions_from_books(books):
    versions = {}
    for b_id in books:
        one_book = books[b_id]
        v_id = one_book['version_id']
        if v_id not in versions:
            versions[v_id] = []
        versions[v_id].append(one_book)
        VersionId_VerName[v_id] = one_book['book_version']
    return versions

def modify_version(ver_name, books, o_subject, o_grade):
    old_ids = ''
    for book in books:
        old_ids += book['old_id'] + ','
    old_ids = old_ids[0:-1]
    get_params = {
        ...
        'subject' : o_subject,
        'grade' : o_grade,
        'bookVersion' : ver_name
    }
    url_parser = urlParser(mfg_config.Modify_Book)
    for pr in get_params:
        url_parser.setParams(pr, get_params[pr])
    result_json = common_down(url_parser)
    if result_json['status'] != '11-001':
...
if __name__ == '__main__':
    ...
    for old_subject in subjects:
        ...
        if len(books) == 0:
            continue
        versions = get_versions_from_books(books)
        for version_id in versions:
            if version_id not in Validate_Versions:
                continue
            ver_books = versions[version_id]
            ver_name = VersionId_VerName[version_id]
            if modify_version(ver_name, ver_books, old_subject, old_grade) != 0:
                Log.error('modify book failed!')
                sys.exit(-1)
            dispose_books(ver_books, old_subject, old_grade)
