...
def crawl_chapter(old_book_id, chapter_params):
    url_parser = urlParser(mfg_config.Text_Book_Chapter)
    for pr in chapter_params:
        url_parser.setParams(pr, chapter_params[pr])
    result_json = common_down(url_parser)
    if result_json['status'] != '11-001':
        ...
        return -1
    book_list = result_json['result']['bookList']
    cur_book = get_select_book(book_list)
    if cur_book['bookId'] == old_book_id:
        chapter_list = result_json['result']['chapterList']
        if chapter_list is None:
            Log.error("get chapter list failed!")
            Log.error(result_json)
    ...
    for book in book_list:
        if book['bookId'] == old_book_id:
            crawl_params = {'bookid' : old_book_id,
              'subject' : chapter_params['subject'],
              'grade' : old_grade,
              'term' : book['term']
              }
            Log.info('now catch book:' + str(book))
            return crawl_one_book(crawl_params, book)
...
def dispose_books(books, subject, old_grade):
    ...
    for cur_book_id in books:
        old_book_id = Books_Info[cur_book_id]['old_id']
        chapter_params['bookId'] = old_book_id
        chapter_list = crawl_chapter(old_book_id, chapter_params)
        dispose_chapter_list(chapter_list, cur_book_id, chapter_params)
...
if __name__ == '__main__':
    ...
    Wantu_File.initial()
    initial_knowledge()
    log_file = '/tmp/crawl.log_normal_tmp'
    ...
    a_book_id = None
    if len(sys.argv) > 2:
        a_book_id = sys.argv[2]

    e_subject = int(a_book_id[3, 4])
    old_subject = mfg_config.compare_check['in_subject'][e_subject]

    old_grade = mfg_config.compare_check['in_grade'][grade_id]

    books = get_book_ids(e_subject, grade_id, int(a_book_id))
    if len(books) == 0:
        sys.exit(0)
    dispose_books(books, e_subject, old_grade)
